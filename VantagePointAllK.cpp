...
float distance(float *pointArray,int index,float *point,int dimension){
   float tmp;
   float dist=0;

  for(int i=0;i<dimension;i++){
        dist += (pointArray[index*dimension+i]-point[i])*(pointArray[index*dimension+i]-point[i]);
        }
  dist=sqrtf(dist);
  return dist;
}
...
void search(struct Node* tree,float* point,int k,int dimension,std::priority_queue<HeapItem>*heap,int pros,int* should){
  ...
  if(tree->leaf==false){
    ...
    if(dist<=tree->radius){
        ...
        t=t<<1;
      if(tree->left!=NULL){
          //printf("--left-- %f,%f \n",tree->vpoint[0],tree->vpoint[1]);
          search( tree->left, point, k,dimension,heap,t,should);
        }else{
            //printf("-------------------------MAMA I JUST KILLED A MAN %d---------------------------\n",t );
            should[t]=1;
          }
      if ( tau>=tree->radius-dist) {
        ...
        //printf("-------------------------PUT A GUN AGAINST HIS HEAD %d---------------------------\n",t );
        should[t]=1;
      }
    }
    }
      ...
      search( tree->right, point, k,dimension,heap,t,should);}
        ...
        //printf("-------------------------PULLED MY TRIGGER NOW HE IS DEAD %d---------------------------\n",t );
        should[t]=1;
      }
      if ( tau>=dist-tree->radius) {
        ...
        //printf("-------------------------MAMA LIFE HAS JUST BEGAN %d---------------------------\n",t );
        should[t]=1;
      }}
...
int main (int argc, char **argv)
{
    ...
    //    printf("size: %d processes: %d\n",size,noProcesses);
        ...
        //  printf("----Creating trees----\n");
          ...
          float* heapdist=(float *)malloc(sizeof(float)*k*partLength);
          float* heapPoints=(float *)malloc(sizeof(float)*dimension*k*partLength);
          ...
          printf("%lu.%lu ", lapsed.tv_sec, lapsed.tv_usec);

          //printf("----Searching trees----\n");
          gettimeofday(&first, &tzp);
          ...
          for(int i=0;i<partLength;i++){
             ...
             *should=0;
              ...
              search(tree,pointsArray+dimension*i,k,dimension,&heap,0,should);
              packHeap(&heap,heapPoints+k*dimension*i,heapdist+k*i,k,dimension);
                 //--------------------------
          }
          ...
          lapsed.tv_usec = second.tv_usec - first.tv_usec;
          lapsed.tv_sec = second.tv_sec - first.tv_sec;
          printf("%d %d %d %lu.%lu ",noProcesses,arg,k, lapsed.tv_sec, lapsed.tv_usec);
    ...
    *temp=localroot;
    ...
    printf("noProcesses:%d SIZE:%d K:%d time%lu.%lu ",noProcesses,arg,k, lapsed.tv_sec, lapsed.tv_usec);
    }
    ...
    float* heapdist=(float *)malloc(sizeof(float)*k*partLength);
    float* heapPoints=(float *)malloc(sizeof(float)*dimension*k*partLength);
    float* tempdist=(float *)malloc(sizeof(float)*k*partLength);
    float* tempPoints=(float *)malloc(sizeof(float)*dimension*k*partLength);
    int* tempshould=(int *)malloc(sizeof(int)*noProcesses*partLength);
    int* processflag=(int *)malloc(sizeof(int)*noProcesses);
    for(int x=0;x<noProcesses;x++){
        processflag[x]=0;
    }

    int* tempprocessflag=(int *)malloc(sizeof(int)*noProcesses);
    //SEARCH YOUR OWN FOR ALL POINTS
    for(int i=0;i<partLength;i++){
      for(int j=0;j<noProcesses;j++)
        {
          should[j+i*noProcesses]=0;
        }
        tau = numeric_limits<float>::max();
        search(&tree,pointsArray+dimension*i,k,dimension,&heap,0,should+i*noProcesses);
        for(int x=0;x<noProcesses;x++){
            processflag[x]=processflag[x]+should[i*noProcesses+x];
        }
  ...
  //SPLIT COMMUNICATION TO LEVEL
    for(int level=q-1;level>=0;level--){
              ...
              //NUMBER OF COMMUNICATION OF EACH LEVEL
              //FIRST HALF SEND-RECEIVE SECOND HALF RECEIVE-SEND
              //TARGET ALL THE NODES OF THE SECOND HALF IN CYCLIC ORDER
              if(local_rank<groupsize/2){
              for(int i=0;i<groupsize/2;i++){

                int target=groupsize/2+(local_rank+i)%(groupsize/2);
                MPI_Send(processflag,noProcesses,MPI_INT,target,0,group);
                MPI_Recv(tempprocessflag,noProcesses,MPI_INT,target,0,group,&Stat);
                if(processflag[(processId -local_rank)+target]>0){
                MPI_Send(heapdist,k*partLength,MPI_FLOAT,target,1,group);
                MPI_Send(heapPoints,partLength*k*dimension,MPI_FLOAT,target,2,group);
                MPI_Send(should,noProcesses*partLength,MPI_INT,target,5,group);
              }
                if(tempprocessflag[processId]>0){
                MPI_Recv(tempdist,k*partLength,MPI_FLOAT,target,3,group,&Stat);
                MPI_Recv(tempPoints,partLength*k*dimension,MPI_FLOAT,target,4,group,&Stat);
                MPI_Recv(tempshould,partLength*noProcesses,MPI_INT,target,6,group,&Stat);
                int sum=0;
                for(int x=0;x<noProcesses;x++){
                    tempprocessflag[x]=0;
                }
                ...
                for(int j=0;j<partLength;j++){
                  if(tempshould[j*noProcesses+processId]==1){
                  sum++;
                  unpackHeap(&heap,tempPoints+dimension*k*j,tempdist+k*j,k,dimension);
                  tau=heap.top().dist;
                  for(int t=0;t<noProcesses;t++)
                    {
                      tempshould[t+j*noProcesses]=0;
                    }
                  search(&tree,tempPoints+dimension*k*j+(k-1)*dimension,k,dimension,&heap,0,tempshould+j*noProcesses);

                  int p=(int)(processId-local_rank+target)/(groupsize/2);
                  for(int t=0;t<noProcesses;t++)
                    {   if((t/(groupsize/2))==p)
                          {tempshould[t+j*noProcesses]=0;}
                    }
                    for(int t=1;t<i;t++){
                    tempshould[(processId+t)%(groupsize/2)+processId-local_rank]=0;
                    }
                    for(int x=0;x<noProcesses;x++){
                        tempprocessflag[x]=tempprocessflag[x]+tempshould[j*noProcesses+x];
                    }
                  packHeap(&heap,tempPoints+k*dimension*j,tempdist+k*j,k,dimension);
                ...
                MPI_Send(tempdist,k*partLength,MPI_FLOAT,target,9,group);
                MPI_Send(tempPoints,partLength*k*dimension,MPI_FLOAT,target,10,group);
                MPI_Send(tempshould,noProcesses*partLength,MPI_INT,target,12,group);}
                if(processflag[(processId -local_rank)+target]>0){
                MPI_Recv(heapdist,k*partLength,MPI_FLOAT,target,7,group,&Stat);
                MPI_Recv(heapPoints,partLength*k*dimension,MPI_FLOAT,target,8,group,&Stat);
                MPI_Recv(should,partLength*noProcesses,MPI_INT,target,11,group,&Stat);
              }
              MPI_Send(tempprocessflag,noProcesses,MPI_INT,target,0,group);
              MPI_Recv(processflag,noProcesses,MPI_INT,target,0,group,&Stat);
              ...
              for(int i=0;i<groupsize/2;i++){
                ...
                MPI_Recv(tempprocessflag,noProcesses,MPI_INT,target,0,group,&Stat);
                MPI_Send(processflag,noProcesses,MPI_INT,target,0,group);
                int pra=tempprocessflag[processId];
                if(tempprocessflag[processId]>0){
                  MPI_Recv(tempdist,k*partLength,MPI_FLOAT,target,1,group,&Stat);
                  MPI_Recv(tempPoints,partLength*k*dimension,MPI_FLOAT,target,2,group,&Stat);
                  MPI_Recv(tempshould,partLength*noProcesses,MPI_INT,target,5,group,&Stat);

                }
                if(processflag[(processId -local_rank)+target]>0){


                MPI_Send(heapdist,k*partLength,MPI_FLOAT,target,3,group);
                MPI_Send(heapPoints,partLength*k*dimension,MPI_FLOAT,target,4,group);
                MPI_Send(should,noProcesses*partLength,MPI_INT,target,6,group);}
                ...
                for(int j=0;j<partLength;j++){
                  ...
                  for(int t=0;t<noProcesses;t++)
                    ...
                    for(int t=1;t<i;t++){
                      tempshould[(processId-t)%(groupsize/2)+processId-local_rank+groupsize/2]=0;
                    }
                    for(int x=0;x<noProcesses;x++){
                        tempprocessflag[x]=tempprocessflag[x]+tempshould[j*noProcesses+x];
                    }
                ...
                }else{    for(int x=0;x<noProcesses;x++){
                        tempprocessflag[x]=tempprocessflag[x]+tempshould[j*noProcesses+x];
                    }}
              }
                //  printf("---------------Sum-%d-------\n",sum );
                ...
                if(processflag[(processId -local_rank)+target]>0){
                MPI_Recv(heapdist,k*partLength,MPI_FLOAT,target,9,group,&Stat);
                MPI_Recv(heapPoints,partLength*k*dimension,MPI_FLOAT,target,10,group,&Stat);
                MPI_Recv(should,partLength*noProcesses,MPI_INT,target,12,group,&Stat);
              }

                if(pra>0){
                MPI_Send(tempdist,k*partLength,MPI_FLOAT,target,7,group);
                MPI_Send(tempPoints,partLength*k*dimension,MPI_FLOAT,target,8,group);
                MPI_Send(tempshould,noProcesses*partLength,MPI_INT,target,11,group);}
                MPI_Recv(processflag,noProcesses,MPI_INT,target,0,group,&Stat);
                MPI_Send(tempprocessflag,noProcesses,MPI_INT,target,0,group);
            ...
            printf("%lu.%lu ", lapsed.tv_sec, lapsed.tv_usec);
                        ...
                        float *bufferPoints,*d;
                        bufferPoints = (float *)malloc(dimension*size*sizeof(float));
                        d = (float *)malloc(size*sizeof(float));
                        //GATHER ALL TO VALIDATE RESULT -> SLOW
                      MPI_Allgather(pointsArray,partLength*dimension,MPI_FLOAT,bufferPoints,partLength*dimension,MPI_FLOAT,MPI_COMM_WORLD);
                      float per;
                      ...
                      MPI_Barrier(MPI_COMM_WORLD);
                         ...
                         //VALIDATION USING QSORT REALY SLOW  partLength*SIZE*LOG(SIZE)
                        for(int p=0;p<partLength;p++){

                          for(int i=0;i<size;i++)
                             {
                             d[i]=distance(bufferPoints,i,pointsArray+p*dimension,dimension);
                             }
                             qsort(d, size, sizeof(float), cmpfunc);



                         for(int j=0;j<k;j++)
                            { if(d[j]>d[j+1]){printf("--------Error--%f--%f--%d \n",d[j],heapdist[-j+(p)*(k)+k-1],processId);}
                            else{
                              if(d[j]!=heapdist[-j+(p)*(k)+k-1]){
                             printf("-------Error--%f--%f--%d \n",d[j],heapdist[-j+(p)*(k)+k-1],processId);}
                            //}
                          }}
...
struct Node* createtreelocal(float *pointsArray,int partLength,int dimension,int processId){
  ...
  //printf("%f %f,%f -\n",median,vpointDim[0],vpointDim[1]);
  //for(int i=0;i<partLength;i++){
  //  printf(" - %f %f,%f-\n",distances[i],pointsArray[i*dimension],pointsArray[i*dimension+1]);
  //}
  //printf("  -\n");
  struct Node *Nodev;
